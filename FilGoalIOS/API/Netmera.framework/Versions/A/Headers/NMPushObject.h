...
typedef enum {
	...
	/// Type for interactive notifications (Interactive notifications received to
	/// devices having iOS7 or before will have type NMNotificationTypeStandard!!)
	NMNotificationTypeInteractive = 1 << 5,
	/// All notification types
	NMNotificationTypeAll = NMNotificationTypePopup | NMNotificationTypeRich | NMNotificationTypeStandard |
	                        NMNotificationTypeURLPush |
	                        NMNotificationTypeInteractive
} NMNotificationType;
...
/**
 * Dictionary which contains custom parameters, which are sent with rich push
 * message content.
 */
@property (nonatomic, strong) NSMutableDictionary *customParams;

/**
 * Dictionary which contains custom parameters sent by the event which
 * auto-triggers this push notification.
 */
@property (nonatomic, strong) NSMutableDictionary *eventCustomParams;

/**
 *  Deeplink url string of the push notification.
 *
 *	@note If the push notification has a deeplink url to be redicted, this value contains the deeplink URL. In such
 *	case, if user taps the push notificaiton message, Netmera will automatically redict to given URL using
 *	[[UIApplication sharedApplication] openURL:] method. All you have to do in such case is interpreting
 *	deeplink url string in -application:openURL:sourceApplication:annotation: method in your application delegate.
 *
 */
 ...
 *
 *      NotificationTypeStandard, plain-text push notification is sent to the
 *relevant devices.
 *
 *      NotificationTypeRich, push notification that contains HTML-formatted
 *rich content is sent to the relevant devices.
 *
 *      NotificationTypePopup, pop-up object that contains HTML-formatted
 *content.
 *
 *      NotificationTypeURLPush, push notification that contains an URL for the
 *rich content is sent to the relevant devices.
 */
...
/**
 * A string value that can be set from Netmera panel while sending push
 *notifications.
 *
...
/**
 * A URL string that can be set from Netmera panel while sending push
 * notifications.
 * You can give logo image paths for the push notification that you sent, and
 * show it in your custom list.
 */
 ...
 *
 * @param handler The block to execute when the details of the push
 *notifications is fetched from Netmera.
 */
...
/**
 * Converts NotificationType enum type into string format that refers to
 *notification type.
 *
...
/**
 *  This property contains value of the "category" key of the "aps" key in push payload if this is an interactive push
 *	notification.
 *	Otherwise, this value is nil.
 */
@property (nonatomic, strong) NSString *interactivePushCategoryIdentifier;

/**
 *  This value is set when there is action specific parameters set in Netmera
 * panel for selected action. If there are no parameters specified for selected
 * action, value returns nil.
 */
@property (nonatomic, strong) NSDictionary *actionParams;

/**
 *  This value is set when user selects an action button for an interactive
 * notification, and contains identifier of selected action. Default is nil.
 */
