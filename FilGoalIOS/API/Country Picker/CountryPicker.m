...
+ (NSDictionary *)countryNamesByCode
{
    ...
    if (!_countryNamesByCode)
    {
        ...
        NSLocale *locale = [NSLocale localeWithLocaleIdentifier:@"ar_EG"];
        ...
        for (NSString *code in [NSLocale ISOCountryCodes])
        {
            ...
            if (!countryName)
            {
                countryName = [[NSLocale localeWithLocaleIdentifier:@"ar_EG"] displayNameForKey:NSLocaleCountryCode value:code];
            }
...
{
    return self.self.countriesList.count;
}
          ...
          forComponent:(__unused NSInteger)component reusingView:(UIView *)view
{
    ...
    Country * country=[[Country alloc]initWithDictionary:(NSDictionary*)[self.countriesList objectAtIndex:row]];
    ((UILabel *)[view viewWithTag:1]).text = country.name;
    NSString *imagePath = country.flag;
    [((UIImageView *)[view viewWithTag:2]) sd_setImageWithURL:[NSURL URLWithString:imagePath]];
       ...
       inComponent:(__unused NSInteger)component
{
  ...
  //  [strongDelegate countryPicker:self didSelectCountryObj:country];
    //[self geocodeCityZoneWithText:self.selectedCountryName];
...
-(void)getCountriesList
{
    ...
    [WServicesManager getDataWithURLString:kGetCountriesList andParameters:dictionary WithObjectName:nil andAuthenticationType:CMSAPIs success:^(NSArray * countries)
     {
