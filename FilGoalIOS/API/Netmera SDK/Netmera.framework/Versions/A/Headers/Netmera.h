...
/**
 * You can set this value to 1 in order to make Netmera utilize
 * advertisingId ([[ASIIdentifierManager sharedManager] advertisingIdentifier]) value to uniquely identify device.
 * If this value is 0, Netmera will user vendorId ([[UIDevice currentDevice] vendorIdentifier]) to identify device.
 */
#define NM_SHOULD_USE_IDFA 0
...
#define NMLocalizedString(key, val)                                                                                    \
	[[NSBundle mainBundle] localizedStringForKey:key value:val table:@"NetmeraLocalization"]
...
/**
 ...
 Default log level is @c NMLogLevelError.

 @param level One of @c NMLogLevel enum values.
 */
...
/**
 * Sets the boolean value that determines whether the push inbox controller is available for your application or not
 *when a rich
...
/**
 * Returns the boolean value that determines whether Netmera should manage application badge number depending on number
 *of unread pushes in push inbox. The default value is YES.
 *
 *  @return The enabled or disabled boolean value.
 */
...
/**
 * Sets the boolean value that determines whether Netmera should manage application badge number depending on number of
 *unread pushes in push inbox.
 ...
 *  receiving rich push is displayed on the screen directly without any alert. If it is NO, then an alert box, which
 *waits
 *  a confirmation about if rich push should be shown or not from the user is shown on the screen. The default value is
 *YES.
 *
 ...
 *    class). If you want to disable tracking user's location, you should call [NetmeraClient
 *setLocationTrackingEnabled:NO]
