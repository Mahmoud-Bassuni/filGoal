...
@interface SampleCustomEventBanner () <SampleBannerAdDelegate,UIWebViewDelegate>
...
- (void)requestBannerAd:(GADAdSize)adSize
  ...
  self.bannerAd =
    ...
    if([json objectForKey:@"mpu"] != nil){
    self.bannerAd.url = [NSURL URLWithString:[json objectForKey:@"mpu"]];
    }
  self.bannerAd.delegate = self;
    //self.bannerAd.webView.delegate = self;
  ...
  [self.bannerAd.webView setBackgroundColor:[UIColor clearColor]];
    [self.bannerAd.webView setOpaque:NO];
}


- (void)bannerDidLoad:(SampleBanner *)banner {
    banner.webView.delegate = _bannerAd;
    [banner.webView loadRequest:[NSURLRequest requestWithURL:banner.url]];
