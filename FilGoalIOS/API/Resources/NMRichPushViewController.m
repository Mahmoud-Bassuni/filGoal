...
#pragma clang diagnostic pop
    ...
    if(![self.richPush isKindOfClass:[NMPopupObject class]])
    {
        ...
        if (![Netmera pushInboxEnabled] || ([Netmera pushInboxEnabled] && !self.richPush.shouldShownInInbox))
            self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NMLocalizedString(@"close_btn", @"Close") style:UIBarButtonItemStylePlain target:self action:@selector(closeButtonTouched)];
    }
    else
    {
