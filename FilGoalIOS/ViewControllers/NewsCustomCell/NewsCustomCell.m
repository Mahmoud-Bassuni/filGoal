...
@interface NewsCustomCell() <UIGestureRecognizerDelegate>

@property (nonatomic, weak) IBOutlet UIButton *deleteBtn;
@property (nonatomic, weak) IBOutlet UIButton *setAsReadBtn;
...
- (IBAction)buttonClicked:(id)sender
{
    ...
    } else if (sender == self.setAsReadBtn) {
        [self.delegate setAsReadAction:self];
    } else {
    }
}
...
{
    return CGRectGetWidth(self.frame) - CGRectGetMinX(self.setAsReadBtn.frame);
}

- (void)panThisCell:(UIPanGestureRecognizer *)recognizer
{
    if(!self.hideDeleteBtn)
    {
    switch (recognizer.state) {
        ...
        case UIGestureRecognizerStateEnded:
            if (self.startingRightLayoutConstraintConstant == 0) { //1
                //We were opening
                CGFloat halfOfButtonOne = CGRectGetWidth(self.deleteBtn.frame) / 2; //2
                ...
                //We were closing
                CGFloat buttonOnePlusHalfOfButton2 = CGRectGetWidth(self.deleteBtn.frame) + (CGRectGetWidth(self.setAsReadBtn.frame) / 2); //4
