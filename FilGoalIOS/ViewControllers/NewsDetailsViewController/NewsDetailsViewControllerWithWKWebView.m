...
@interface NewsDetailsViewControllerWithWKWebView ()<AMScrollingNavbarDelegate>
{
...
@implementation NewsDetailsViewControllerWithWKWebView
#define PADDING 30
...
- (void)viewDidLoad
{
    ...
    UIView * footerView=[[UIView alloc]initWithFrame:CGRectMake(0,8, Screenwidth, 270)];
    ...
    [_sponsorFirstView addSubview:footerView];
    
    
    
    
    
    //self.sponsorFirstView = self.tableView.tableHeaderView;
...
{
    UIView * footerView=[[UIView alloc]initWithFrame:CGRectMake(self.sponsorFirstView.frame.origin.x,self.sponsorFirstView.frame.origin.y, Screenwidth, 270)];
    [footerView setBackgroundColor:[UIColor clearColor]];
    GADAdSize customAdSize = GADAdSizeFromCGSize(CGSizeMake(300, 250));
    DFPBannerView * bannerView=[[DFPBannerView alloc]initWithAdSize:customAdSize origin:CGPointMake((Screenwidth-300)/2-10, 0)];
    [footerView setBackgroundColor:[UIColor lightGrayAppColor]];
    ...
    [list addObjectsFromArray:@[@"Inner",@"Article"]];
    [super setscreenAnalyticsMetricsWithScreenName:[NSString stringWithFormat:@"iOS- News Details with News ID =%i",self.newsItem.newsId ] andKeywords:[list componentsJoinedByString:@","]];
    request.customTargeting = [[NSDictionary alloc]initWithObjects:@[list,@[@"Pos2"]] forKeys:@[@"Keyword",@"Position"]];
...
-(NSArray*)getSectionNamesList
{
    ...
    return [[NSArray alloc]initWithArray:sectionNames];
}
...
//    NSLog(@"%@",GetAppDelegate().getSelectedNavigationController.viewControllers);
    if([self.navigationController.topViewController isKindOfClass:[NewsDetailsViewControllerWithWKWebView class]])
    {
...
-(void)loadViewsFromNewsItemWithRelatedList:(int)currentList{
    ...
    self.webView.configuration.preferences=prefs;
...
//       strBody= [NSString stringWithFormat:@"<style>body { background-color: trasparent; font-size:%ipx; color: %@; margin:0; font-family:\"%@\"} a { color: #172983; }img{width:100%%; height:auto}iFrame{ width : 100%%}</style><body><div id='content' dir='rtl' name='content'>%@</div></body>",self.fontSize,self.textColor, @"DINNextLTW23Regular",self.newsItem.newsDescription];
    ...
    //  self.webView.scrollView.tag = 20;
...
#ifdef DEBUG
    NSLog(@"viewDidAppear newsVC --- pageIndex: %ld, \tnewsId: %ld, \tnewsTitle: %@", (long)self.pageIndex, (long)self.newsItem.newsId, self.newsItem.newsTitle);
#endif
...
-(void)setCoSponsorScreenSponsor
{
    //Testing
#ifdef DEBUG
    self.sponser.hidden = YES;  //Release
    //self.sponser.hidden = NO; //Testing
    ...
    for (NewsSectionId *nsec in self.newsItem.newsSectionId) {
        if([AppSponsors isSectionCoSponsorActiveUsingId:nsec.sectionId]) {
            ...
            //Testing
            //sponsorUrl = @"https://api.filgoal.com/MobileAppResources/iOS/RussiaSponsorShips/SponsorShip12/EG/mainSponsor/specialSponsor/champ/collpased/750-782@2x.png";
    ...
    //Testing self.sponser.hidden=NO;
    for (NewsSectionId *nsec in self.newsItem.newsSectionId) {
        if([AppSponsors isSectionCoSponsorActiveUsingId:nsec.sectionId])
        {
            self.sponser.tapURL = [AppSponsors activeSectionCoSponsorTapUrlUsingId:nsec.sectionId category: @"News"];
            sponsorUrl=[NSString stringWithFormat:@"%@",[AppSponsors getSectionBannerStripeImagePathUsingSectionId:nsec.sectionId andContentType:@"News"]];
            //TESTING sponsorUrl = @"https://api.filgoal.com/MobileAppResources/iOS/RussiaSponsorShips/SponsorShip12/EG/mainSponsor/specialSponsor/champ/collpased/750-782@2x.png";//
            [self.sponser sd_setImageWithURL:[NSURL URLWithString:sponsorUrl] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if(error)
                {
                    self.sponser.hidden=YES;
...
- (void)webView:(WKWebView *)aWebView didFinishNavigation:(WKNavigation *)aNavigation
{
    ...
    [aWebView evaluateJavaScript:@"document.body.scrollHeight;" completionHandler:^(id result, NSError *error) {
        if (result != nil) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if (dic.allKeys.count>0 &&[[dic.allKeys objectAtIndex:0] isEqualToString:@"مقالات أخري للكاتب"]&& [[list objectAtIndex:indexPath.row]isKindOfClass:[NewsItem class]])
    {
        ...
        NSArray *viewControllers = [NSArray arrayWithObject:[[NewsDetailsViewControllerWithWKWebView alloc]initWithNewsItem:indexPath.row withNewsList:list]];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(UIViewController *)viewController {
    ...
    if (self.currentRelatedList==1) {
        ...
        int index = [(NewsDetailsViewControllerWithWKWebView *)viewController currentNewsIndex];
        index--;
        ...
        if (index == NSNotFound||index<0) {
            ...
            NewsDetailsViewControllerWithWKWebView *childViewController = [[NewsDetailsViewControllerWithWKWebView alloc] initWithNewsItem:index withNewsList:itemsWithAdsArray];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerAfterViewController:(UIViewController *)viewController{
    ...
    if (self.currentRelatedList==1) {
        ...
        if (index == NSNotFound||index>=self.relatedNewsList.count) {
            ...
            NewsDetailsViewControllerWithWKWebView *childViewController = [[NewsDetailsViewControllerWithWKWebView alloc] initWithNewsItem:index withNewsList:self.relatedNewsList];
            return childViewController;
        }
...
- (NewsDetailsViewControllerWithWKWebView *)viewControllerAtIndex:(NSUInteger)index {

    NewsDetailsViewControllerWithWKWebView *childViewController = [[NewsDetailsViewControllerWithWKWebView alloc] initWithNewsItem: index withNewsList: self.orginalNewsList];
    return childViewController;
                       ...
                       context:(void *)context
{
  //  NSLog(@"%f",webViewHeight);
    if (object == self.webView.scrollView && [keyPath isEqual:@"contentSize"]&&self.webView.scrollView.contentSize.height>webViewHeight&& webViewHeight != 0) {
